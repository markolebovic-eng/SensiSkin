...
## Post 2661 — flagship estetski/H2 prolaz (2026-07-13)
- ODLUKA VLASNIKA: /hydrafacial-mesecna-rutina/ (post 2661) je NOVI referentni blog post — zamenjuje /leto-i-promene-na-kozi/ kao šablon za stil budućih postova. Ubuduće koristiti PRAVE H2/H3 (ne bold-pasus pseudo-podnaslove) za nove postove; postojeći bold-pasus postovi se NE diraju dok se ne odluči šire.
- BACKUP: puno stanje posta PRE estetskog prolaza sačuvano na /outputs/sensiskin/wp-backups/2661-2026-07-13-pre-aesthetic-pass.json (commit-ovano u repo, ne u temp — trajno, preživljava sesiju/mašinu). Za rollback: PATCH sadržaj tog fajla nazad na post 2661.
- IZMENE PRIMENJENE (status ostao private tokom svega): 9 bold-pseudo-podnaslova → pravi <h2> centrirano (jedan već sadrži focus keyphrase "HydraFacial mesečna rutina" — sad je i semantički H2, ne samo bold); drop cap na uvodni pasus; postojeća samostalna rečenica pretvorena u pullquote (bez novog teksta); postojeća bullet lista upakovana u "key takeaways" kutiju (brend boje sa Blocksy teme: #F0F1F3 pozadina, #b92c3e border-left akcenat — NE izmišljene boje, izvučene iz live global.css); nova VIDLJIVA FAQ sekcija (H2 "Najčešća pitanja" + 5× H3 pitanje/pasus odgovor) — tekst verbatim identičan postojećoj JSON-LD FAQPage schemi (koja ostaje nepromenjena, invisible); završni CTA + brend-kredibilitetna rečenica centrirani.
- Slike (2672, 2673) ostale iz prošlog prolaza: 800x533, centrirane, zaobljene ivice 8px — netaknute ovim prolazom.
- Istraženo (WebSearch): strukturni obrasci vrhunskih beauty/wellness blogova (Healthline, Byrdie) — key takeaways kutija sa vizuelnim isticanjem, pull quote na svakih 300-400 reči, FAQ kao bold pitanje + kratak odgovor bez accordion-a. Primenjeno kroz native Gutenberg blokove (nema pristupa CSS/temi, samo post content).
- OTVORENO: vlasnik treba da pregleda u wp-adminu; ako se ne dopadne, rollback iz backup fajla je spreman. Ako se dopadne, sledeći korak je preneti isti H2/H3 standard i na ostale nove postove (i razmotriti postepeno na stare).
